...
<main class="container">
  ...
  <form action="../back/controllers/expenses_insert.php" method="post" class="card" novalidate>
    <?= csrf_field() ?>
    <div class="field">
      <label for="date">Fecha</label>
      <input type="date" id="date" name="date" required
             value="<?= htmlspecialchars($date) ?>" pattern="\d{4}-\d{2}-\d{2}">
    </div>
    </div>
    </div>
    ...
    <div class="field">
      ...
      <input type="text" id="concept" name="concept" required maxlength="180"
    ...
    <div class="field">
      ...
      <input type="text" id="category" name="category" required maxlength="60"
             value="<?= htmlspecialchars($category) ?>" placeholder="Ej.: Alimentación">
    </div>

    <div class="field">
      <label for="amount">Importe (€) - usa punto decimal (ej.: 12.50)</label>
      <input type="number" id="amount" name="amount" required step="0.01"
             inputmode="decimal" value="<?= htmlspecialchars($amount) ?>">
      <small class="help">Gasto &gt; 0, ingreso &lt; 0 (opcional).</small>
    </div>

    <div class="field actions">
      <button class="btn primary" type="submit" aria-label="Guardar gasto">Guardar</button>
      <a class="btn" href="index.php" aria-label="Cancelar y volver al listado">Cancelar</a>
    </div>
  </form>
